...
namespace winrt::vertical_tasks::implementation
{
    struct WindowProcess
    {
        ...
        void GetNewWindowProcess(DWORD pid, DWORD tid, std::wstring name);         // Initializes a new instance of the <see cref="WindowProcess"/> class
        ...
        DWORD GetProcessIDFromWindowHandle(HWND hwnd);                            /// Gets the process ID for the window handle
        DWORD GetThreadIDFromWindowHandle(HWND hwnd);                             /// Gets the thread ID for the window handle
        std::wstring GetProcessNameFromProcessID(DWORD processId);                /// Gets the process name for the process ID
